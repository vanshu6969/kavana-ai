...
export default function ChatsPage() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-24 space-y-6">
      ...
      {/* Active Conversation Rows / Grid or Empty State */}
      {sessions.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center bg-[#0D0E15] border border-white/[0.08] rounded-3xl space-y-4">
          <div className="w-16 h-16 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-center text-[#FF2E55] shadow-glow-crimson">
            <MessageSquare size={30} />
          </div>
          <div className="space-y-1">
            <h3 className="text-lg sm:text-xl font-black text-white">No Active Story Chats Yet</h3>
            <p className="text-xs sm:text-sm text-slate-400 max-w-md leading-relaxed">
              You haven&apos;t started any story roleplays yet. Pick any story or drama from the home feed to begin your custom adventure.
            </p>
          </div>
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#FF2E55] to-[#E00034] text-white text-xs sm:text-sm font-bold shadow-glow-crimson hover:scale-105 transition-all"
          >
